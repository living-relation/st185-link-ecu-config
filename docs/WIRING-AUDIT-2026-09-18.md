...
**Pass A COMPLETE** (2026-09-18, branch `pass-a-bh-c-delete-locks`):
...
- B29 / DI8: IO table locked **VSS from trans for mph** (trans cavity not drawn — no invented pins)
...
| OPEN | B29 harness wire | IO locked; **OEM VSS cavity TBD from EWD** — do not invent |
| OPEN | Fuel divider Ω | Approach locked; value TBD for BOM/calibration |

## Draft (not committed)
`C:\Users\danie\AppData\Local\Temp\st185-audit\ecu_pin_graph_DRAFT.csv` — crown **after** Pass A (optional this pass).
...
7. Wire B29 when OEM VSS cavity confirmed; set fuel-divider Ω at calibration
...
- **Pin-graph CSV** — commit **after** Pass A (still optional next)
...
- **B29** — **VSS from trans for mph** (IO locked; harness cavity TBD)
- **Fuel level** — OEM sender + **voltage divider** (Ω TBD)
- **Git** — **feature branch only**; no main until verified

## Open locks (Daniel)
None on policy. Remaining **build** openers: OEM VSS cavity for B29; fuel-divider Ω for BOM. Writer: **Grimoire only**, local DansPC, branch `pass-a-bh-c-delete-locks`. Main untouched until verified. Cloud Agent jarred. No dual-cast.
