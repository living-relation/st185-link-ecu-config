...
| `2026-09-25-cleanup/` | 2026-09-25 | Wiring clutter, at Daniel's direction: keep only the SoT (`sot/channels.csv`), the looms (`docs/harness/rebuild/`) and `XTREMEX-IO-TABLE.html`. Holds the retired print/interactive schematics (`SCHEMATIC-WIRING.html`, `apps/harness-schematic/`, `apps/harness-overlay/` - all drawn from the old Power/Signal files), the dated VERIFY/AUDIT/FACES/BOARD-VERIFY notes, `POWER-AND-COMMS-ARCHITECTURE.md` (still planned a PDU; 6.48 dropped it), the two loose root notes, and the one-shot `fix_*`/`gen_*` scripts and screenshots from `docs/harness/`. Paths under the folder mirror where each file used to live. |
